...
export const Peserta = () => {
  ...
  const calculateUserStatus = (user) => {
    ...
    // If status_aktif is none of the above and tanggal_selesai is before today, return "Alumni"
      ...
      moment(user.tanggal_selesai).isBefore(today)
    ) {
      return "Alumni";
    }
  ...
  const validateForm = () => {
    ...
    if (!formData.tanggal_mulai || mulaiDate < currentDate) {
      errors.tanggal_mulai =
        "Tanggal mulai harus diisi minimal tanggal hari ini!";
      isValid = false;
    } else {
      errors.tanggal_mulai = "";
    }
    ...
    if (!formData.tanggal_selesai || selesaiDate < thirtyDaysAfterMulai) {
      errors.tanggal_selesai =
        "Tanggal selesai harus diisi minimal 30 hari dari tanggal mulai!";
      isValid = false;
    } else {
      errors.tanggal_selesai = "";
    }
  ...
  const saveUser = async (e) => {
    ...
    try {
      const response = await axiosJWTadmin.post(
